var questionNum, timeToAnswer, selected = '', winnings, timeSaved,
    version, money, ladderHTML, currentMessage,
    usedFiftyFifty = 0, usedPhoneAFriend = 0, usedAskAudience = 0;
...
function areYouSure() {
  $('#message-text').html((money[questionNum-1] > 0 || currentMessage == 'walk?')? 'Walk away with £'+(money[questionNum-1]||0)+'?':'Are you sure?');
...
function questionFailure() {
  ...
  $('#quit').css('visibility','hidden');
  $('#next-question').css('visibility','hidden');
  highScore()? newHighScore() : $('#confirm').html('Back to Menu').css('visibility','inherit');
  $('#message-board').css('visibility','visible');
  $('input').css('visibility','hidden');
}
...
$('#confirm').click(function(){
  if (currentMessage == 'phone') startTimer();
  if (currentMessage == 'help' || currentMessage == 'phone') { $('#message-board').css('visibility','hidden'); return; }
